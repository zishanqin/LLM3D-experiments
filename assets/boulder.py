...
class BoulderFactory(AssetFactory):
    ...
    def __init__(
        ...
        self.octree_depth = 3 if 'octree_depth' not in control_dict else control_dict['octree_depth']
        # octree depth range [1, 2, 3, 4]
        self.do_voronoi = do_voronoi if 'do_voronoi' not in control_dict else control_dict['do_voronoi']
        # do_voronoi range [True, False]
        self.weights = [.8, .2]
        self.configs = ['boulder', 'slab']
        with FixedSeed(factory_seed):
            self.rock_surface = surface.registry('rock_collection')
            if 'config' in control_dict:
