...
class FlowerPlantFactory(AssetFactory):
    ...
    def control(self, control_dict):
        ...
        if 'leaves_version' in control_dict:
            # v1: not smooth
            # v2: smooth and larger in size
            # v3: more standard and flat
            # v4: giant version of v3
            # v5: triangular shape
            # v6: more rounded shape visually
            self.leaves_version_num = control_dict['leaves_version']
        if 'flowers_version' in control_dict:
            # v1: big polygen shape
            # v2: small to middle-sized rounded flower
            # v3: similar to v2
            # v4: similar to v2 but less smooth
            # v5: giant version of v4
            self.flowers_version_num = control_dict['flowers_version']

        # Control size
        if 'leaf_width' in control_dict: # control size of leaves
            self.leaf_width = control_dict['leaf_width']
        else:
            self.leaf_width=0.35
        
        if 'rad_size' in control_dict: # control size of flowers
            self.rad = control_dict['rad']
        else:
            self.rad = uniform(0.4, 0.7)
