...
def build_scene_asset(factory_name,control,control_dict, idx):
    factory = None
    ...
    with FixedSeed(idx):
        # TODO: check here, if we want make control, the 'control' and 'control_dict' need set here,
        #  when the factory been initialized. Avoid global variable outside the class.
        factory = factory(idx,control=control,control_dict=control_dict)
...
def build_scene_surface(factory_name,control,control_dict, idx):
    try:
        with gin.unlock_config():
...
def build_scene(path, idx, factory_name,control,control_dict, args):
    ...
    if 'Factory' in factory_name:
        asset = build_scene_asset(factory_name,control,control_dict, idx)
    else:
        asset = build_scene_surface(factory_name,control,control_dict, idx)
...
def make_args():
    ...
    parser.add_argument("-m", '--margin', default=.1,
                        help="Margin between the asset the boundary of the image when automatically adjusting "
                             "the camera")
    parser.add_argument('-r', '--resolution', default='256x256', type=str,
                        help="Image resolution widthxheight")
    parser.add_argument('-p', '--samples', default=128, type=int, help="Blender cycles samples")
...
def test(args,control,control_dict):
    ...
    for factory in factories:
        ...
        for idx in range(args.n_images):
            try:
                build_scene(fac_path, idx, factory,control,control_dict, args)
                # exit()
            except Exception as e:
                print(e)
                continue
