...
class DandelionFactory(AssetFactory):
    ...
    def control(control_dict):
        ...
        if 'single_mode' in control_dict:
            ...
            # the first one 'mode' stores the list of mode names
            # the second one 'pb' stores the list of probabilities correpondingly
            self.flower_mode = control_dict['multi_mode']['mode']
            self.flower_mode_pb = control_dict['multi_mode']['pb']
        elif 'customized_mode' in control_dict:
            # customized mode - allows you to control the random_dropout, row and cols freely as you want
            # a dict can be used to directly control the params
            self.customized_params = control_dict['customized_mode']
        else: # no control - randomly selection based on weights
            self.flower_mode = ['full_flower', 'no_flower', 'top_half_flower', 'top_missing_flower', 'sparse_flower']
            self.flower_mode_pb = [0.4, 0.04, 0.23, 0.13, 0.2]
    ...
    def get_mode_params(self, mode):
        if self.customized_params is not None:
            return self.customized_params
        if mode == 'full_flower':
