...
def control(control_dict):
    global resolution_control, cloud_size_control, cloud_quantity_dict
    global total_sample_num_control, preview_sample_num_control
    global grid_size_control, single_cloud_type, specific_configs

    # Resolution testing
    if 'resolution' in control_dict:
        # should be set based on the input resolution number
        resolution_control = control_dict['resolution']
    else:
        resolution_control = 256 # default as 256
    
    # Cloud size testing
    if 'cloud_size' in control_dict:
        # range = [16, 32, 64, 128, 256, 512]
        cloud_size_control = control_dict['cloud_size']
    else:
        cloud_size_control = 128 # default as 128


    # Sample size testing
    # TODO: need to check the real differences between preview_samples and sample_num_control via some testings
    if 'sample_num' in control_dict:
        # range = [64, 128, 256, 512, 1024]
        total_sample_num_control = control_dict['sample_num']
    
    if 'preview_sample_num' in control_dict:
        # size <= total_sample_num_control
        # range = [8, 16, 32, 46]
        preview_sample_num_control = control_dict['preview_sample_num']

    # Cloud type and number testing
    # Multiple cloud testing
    if 'cloud_dict' in control_dict:
        # range for all different cloud types:
        # 'Cumulus': choosing from [0, 2, 4 ,5, 6, 8, 10]
        # 'Cumulonimbus': choosing from [0, 2, 4 ,5, 6, 8, 10]
        # 'Stratocumulus': choosing from [0, 2, 4 ,5, 6, 8, 10]
        # 'Altocumulus': choosing from [0, 2, 4 ,5, 6, 8, 10]
        cloud_quantity_dict = control_dict['cloud_dict']
    # Single cloud testing
    if 'single_cloud' in control_dict:
        single_cloud_type = control_dict['single_cloud']

    # Grid size testing
    if 'grid_size' in control_dict:
        # range = [64, 128, 256, 512, 1024]
        grid_size_control = control_dict['grid_size']
        

    # Testing on each specific type
    if 'specific_config' in control_dict:
        # the range for the key 'scale' is [512, 1024] for Cumulonimbus and [16, 32] for the rest
        for typ in control_dict['specific_config']:
            specific_configs[typ] = control_dict['specific_config'][typ]        
...
def initialize(collection):
    ...
    bpy.context.scene.cycles.preview_samples = sample_num_control
    bpy.context.scene.cycles.samples = preview_sample_num_control

    grid_size = grid_size_control

    xs = np.linspace(-grid_size, grid_size, 5)
    ys = np.linspace(-grid_size, grid_size, 5)
    xs, ys = np.meshgrid(xs, ys)
    xs = xs.reshape(-1)
    ys = ys.reshape(-1)

    size = cloud_size_control
    ...
    clouds += [Cumulus(f'Cumulus_{i:03d}', ref_cloud).make_cloud(marching_cubes=False, resolution=resolution, )
        for i in range(cloud_quantity_dict['Cumulus'])]
    clouds += [Cumulonimbus(f'Cumulonimbus_{i:03d}', ref_cloud).make_cloud(marching_cubes=False,
        resolution=resolution, ) for i in range(cloud_quantity_dict['Cumulonimbus'])]
    clouds += [Stratocumulus(f'Stratocumulus_{i:03d}', ref_cloud).make_cloud(marching_cubes=False,
        resolution=resolution, ) for i in range(cloud_quantity_dict['Stratocumulus'])]
    clouds += [
        Altocumulus(f'Altocumulus_{i:03d}', ref_cloud).make_cloud(marching_cubes=False, resolution=resolution, )
        for i in range(cloud_quantity_dict['Altocumulus'])]
...
def single():
    ...
    bpy.context.scene.cycles.preview_samples = preview_sample_num_control
    bpy.context.scene.cycles.samples = total_sample_num_control
    ...
    ref_grid = create_3d_grid(steps=grid_size_control)
    ref_cloud = bpy.data.meshes.new('ref_cloud')
    ref_cloud.from_pydata(ref_grid, [], [])
    ref_cloud.update()

    if single_cloud_type == 'Cumulonimbus':
        clouds = [Cumulonimbus(f'Cumulonimbus_{i:03d}', ref_cloud).make_cloud(marching_cubes=True, resolution=128, )
            for i in range(1)]
    elif single_cloud_type == 'Cumulus':
        clouds = [Cemulus(f'Cumulus_{i:03d}', ref_cloud).make_cloud(marching_cubes=False,
        resolution=resolution, ) for i in range(1)]
    elif single_cloud_type == 'Stratocumulus': 
        clouds = [Stratocumulus(f'Stratocumulus_{i:03d}', ref_cloud).make_cloud(marching_cubes=False,
        resolution=resolution, ) for i in range(1)]
    elif single_cloud_type == 'Altocumulus': 
        clouds = [Altocumulus(f'Altocumulus_{i:03d}', ref_cloud).make_cloud(marching_cubes=False,
        resolution=resolution, ) for i in range(1)]
